            ...
            try {
                ...
                groups.filter(grp => grp.commands.some(cmd => !cmd.hidden && cmd.isUsable(msg)))
                    .map(grp => {
                        ...
                        .setAuthor(`Cleaner`, `https://i.ibb.co/sPNkqzb/pngwing-com.png`)
                    });

                messages.push(await msg.direct('', embed));
                if (msg.channel.type !== 'dm') messages.push(msg.channel.send('', { embed: { color: '00ff00', title: `**Success!** <:tick:788102360936218685>`, description: `Sent help documentation to ${msg.author}`} }).then(m => m.delete({ timeout: 5000, reason: 'Automated deletion.' }))) //successfully sent help, notify them in the guild.
            } catch (err) {
